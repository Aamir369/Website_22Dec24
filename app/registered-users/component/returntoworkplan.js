...
export default function ReturnToWorkPlan() {
    ...
    return (
        <div className="p-8 max-w-2xl mx-auto bg-white border-4 border-black border-double rounded-lg shadow-lg">
            <h1 className="text-2xl font-bold text-center mb-4">RETURN TO WORK PLAN</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                {workers.map((worker, index) => (
                    <div key={index} className="p-4 border rounded bg-gray-50 space-y-4">
                        ...
                        <div>
                            <label className="block font-semibold mb-1">Return completed form to:</label>
                            <input
                                type="text"
                                name="returnToEmail"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block font-semibold mb-1">Name of Injured Worker</label>
                                <input
                                    type="text"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block font-semibold mb-1">Department / Area</label>
                                <input
                                    type="text"
                                    name="departmentArea"
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <label className="block font-semibold mb-1">YOU HAVE BEEN SCHEDULED TO RETURN TO WORK ON:</label>
                        ...
                        {/* Employee Status */}
                        <div>
                            <label className="block font-semibold mb-1">THE EMPLOYEE IS: <span className="italic">(check all that apply)</span></label>
                            <div className="space-y-2">
                                {[
                                    ...
                                    <label key={i} className="flex items-center bg-[#edf2f9] border p-2 rounded">
                                        <input
                                            type="checkbox"
                                            checked={worker.employeeStatus.includes(option)}
                                            onChange={() => handleCheckboxChange(index, 'employeeStatus', option)}
                                            className="mr-2"
                                        />
                                        {option}
                                    </label>
                        ...
                        <div className="grid grid-cols-4 gap-4 items-center bg-[#edf2f9] border p-2 rounded">
                            <label className="flex items-center col-span-1">
                                <input
                                    type="checkbox"
                                    checked={worker.employeeStatus.includes('Working a partial day')}
                                    onChange={() => handleCheckboxChange(index, 'employeeStatus', 'Working a partial day')}
                                    className="mr-2"
                                />
                        ...
                        {/* Review Checklist */}
                        <div>
                            <label className="block font-semibold mb-1">THE FOLLOWING REVIEW AND BRIEFING HAS OCCURRED</label>
                            <div className="space-y-2">
                                {[
                                    ...
                                    <label key={i} className="flex items-start bg-[#edf2f9] border rounded p-2">
                                        <input
                                            type="checkbox"
                                            checked={worker.reviewChecklist.includes(item)}
                                            onChange={() => handleCheckboxChange(index, 'reviewChecklist', item)}
                                            className="mr-2 mt-1"
                                        />
                                        <span>{item}</span>
                                    </label>
                        ...
                        <p className="text-sm leading-relaxed">
                            ...
                            I, the undersigned injured worker, agree to participate in the transitional work plan described herein. I agree to consider work to be performed carefully and to work within my restrictions, ask for help when work exceeds my abilities, to notify my supervisor if there are duties assigned that exceed my abilities, or if I need assistance.
                        </p>
                        ...
                        <div className="border border-gray-400">
                            ...
                            <div className="grid grid-cols-3">
                                <input type="text" name="employeeNameAgreement" value={worker.employeeNameAgreement} onChange={(e) => handleChange(index, e)} placeholder="Employee Name" className="border-t border-r p-2 bg-[#edf2f9]" />
                                <input type="text" name="employeeSignature" value={worker.employeeSignature} onChange={(e) => handleChange(index, e)} placeholder="Employee Signature" className="border-t border-r p-2 bg-[#edf2f9]" />
                                <input type="date" name="employeeDate" value={worker.employeeDate} onChange={(e) => handleChange(index, e)} className="border-t p-2 bg-[#edf2f9]" />
                            </div>
                            <div className="grid grid-cols-3">
                                <input type="text" name="supervisorNameAgreement" value={worker.supervisorNameAgreement} onChange={(e) => handleChange(index, e)} placeholder="Supervisor Name" className="border-t border-r p-2 bg-[#edf2f9]" />
                                <input type="text" name="supervisorSignature" value={worker.supervisorSignature} onChange={(e) => handleChange(index, e)} placeholder="Supervisor Signature" className="border-t border-r p-2 bg-[#edf2f9]" />
                                <input type="date" name="supervisorDate" value={worker.supervisorDate} onChange={(e) => handleChange(index, e)} className="border-t p-2 bg-[#edf2f9]" />
                            </div>
                        </div>
                    </div>
                ))}
                ...
                <div className="flex justify-between pt-4">
                    <button type="submit" className="bg-blue-700 text-white px-8 py-2 rounded hover:bg-blue-800">SUBMIT</button>
                    <button type="button" onClick={addWorker} className="bg-blue-700 text-white px-8 py-2 rounded hover:bg-blue-800">ADD WORKER</button>
                </div>

                {success && <p className="text-green-600 mt-4">Plan submitted successfully!</p>}
            </form>
        </div>
    );
}
